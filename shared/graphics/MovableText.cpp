...
void MovableText::getWorldTransforms(Matrix4 *xform) const 
{
    if (this->isVisible() && mpCam)
    {
        ...
        Ogre::Vector3 pos   =  mParentNode->getPosition();
        Ogre::Vector3 right =  mParentNode->getLocalAxes().GetColumn(0);
        Ogre::Vector3 up    =  mParentNode->getLocalAxes().GetColumn(1);
        Ogre::Vector3 fwd   =  mParentNode->getLocalAxes().GetColumn(2);

        right.normalise();
        up.normalise();
        fwd.normalise();

        Ogre::Vector3 ppos = pos + right * mGlobalTranslation.x + up * mGlobalTranslation.y + fwd * mGlobalTranslation.z;
