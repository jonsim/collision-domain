...
void GraphicsApplication::createScene (void)
{
	...
	//We're going to get the main camera to look down at our entire plane
	Ogre::Entity* groundEnt = mSceneMgr->getEntity("Ground");
	Ogre::AxisAlignedBox groundBox = groundEnt->getBoundingBox();
	Ogre::Vector3 gbCenter = groundBox.getCenter(); //Move camera to center
	Ogre::Vector3 camPos = groundBox.getCenter(); //Move camera to center
	//When thinking about mCamera->FOVy remember it's 2D
	Ogre::Vector3 maxDisplacement = groundBox.getMaximum() - gbCenter;
	//float newY = (1.0f / (float)tan(mCamera->getFOVy().valueRadians() / 2.0))*(maxDisplacement.length());
	float newY = maxDisplacement.length()/(float)tan(mCamera->getFOVy().valueRadians());//Works out the better ratio we need to use
	camPos.y = newY;//Move it up a lot
	mCamera->setPosition(camPos);
	mCamera->lookAt(ninjaEntity->getBoundingBox().getCenter());
