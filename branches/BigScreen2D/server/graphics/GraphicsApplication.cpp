...
void GraphicsApplication::setupArena (void)
{
    ...
    arenaEntity = GameCore::mSceneMgr->createEntity("Arena", "arena.mesh");
    arenaEntity->setMaterialName("arena_uv");
    arenaEntity->setCastShadows(true); // without shadows you can't see the seating rows
 
	/*
	std::stringstream tmpDebugStream;
	tmpDebugStream << "X: " << arenaSize.x;
	tmpDebugStream << "Y: " << arenaSize.y;
	tmpDebugStream << "Z: " << arenaSize.z;
	tmpDebugStream << "\n";
	
	OutputDebugString(tmpDebugStream.str().c_str());
	*/
